...
### 3.2 型テンプレを使う（`pages.py` の28型・コーススライド）

コーススライドは、型テンプレ集 v0.4 の全28型を **`pages.py`（リポジトリ直下）に関数化済み**。content.txt のページ見出しに **`【型名】`**（和名 or idx）があれば、対応する `pages.py` 関数を呼ぶだけでブランド準拠・ゾーン内に収まる。判定表は次の通り（フィールドは `キー: 値`、複数値は `｜` 区切り／リスト。`型名=説明` 形式のペアも可）。**`【型名】` が無いページは従来どおり §3 の自由設計**でよい。

| idx | 【型名】(和名) | `pages.py` 関数 | 主なフィールド |
|---|---|---|---|
| A1 | コース表紙 | `course_cover` | title, subtitle, meta |
| A2 | カリキュラム/目次 | `curriculum` | title, items（章=概要） |
| A3 | チャプター扉 | `chapter_cover` | num, course_name, icon, goals |
| B1 | レッスン表紙 | `lesson_cover` | title, subtitle, lead |
| B2 | 学習目標 | `objectives` | title, goals（※標準は章扉に内蔵） |
| B3 | 導入/動機づけ | `motivation` | title, lead, points, conclusion |
| B4 | 前提知識/前提 | `prerequisites` | title, terms（用語=意味） |
| C1 | 概念解説/概念 | `concept` | title, lead, flow, conclusion |
| C2 | 手順/ステップ | `steps_page` | title, items, conclusion |
| C3 | 具体例/ケース | `example` | title, scenario, points, conclusion |
| C4 | 比較/選択 | `comparison` | title, headers, rows, conclusion |
| C5 | データ/エビデンス | `data` | title, chart(bar/line/pie), categories, series/values |
| C6 | 構造/関係図 | `structure` | title, groups｜cycle｜pyramid |
| C7 | よくある失敗/注意 | `pitfalls` | title, bad, good |
| C8 | 画像解説/図版 | `image_explain` | title, image, heading, bullets, side |
| C9 | 画面操作/UIガイド | `ui_guide` | title, image, pins("x,y")｜steps |
| C10 | コード解説/コード | `code_explain` | title, code, code_title, highlight, notes |
| C11 | 例文/対訳 | `parallel_text` | title, pairs（原文=訳） |
| C12 | 会話/ダイアログ | `dialog` | title, turns（話者:台詞:side） |
| C13 | 文法/構文 | `grammar` | title, terms, ops, examples |
| C14 | 語彙/用語リスト | `vocabulary` | title, words（語=意味[=例]） |
| C15 | テンプレート/書式 | `format_template` | title, template, fill_ins |
| D1 | ワーク/実践 | `exercise` | title, task, hints |
| D2 | 理解チェック/クイズ | `quiz` | title, question, options, answer, explanation |
| D3 | チェックリスト | `checklist` | title, items |
| D4 | まとめ | `summary` | title, points, takeaway |
| D5 | 次回予告/さらに学ぶ | `next_up` | title, next_topic, resources |
| D6 | 修了/行動喚起 | `complete` | title, message, cta |

呼び出しは `import pages as P` して `P.関数(prs, ...)`、または `pages.render(prs, "【型名】oridx", fields_dict)`。生成された各ページも §5 の検査ゲート・design-guide 監査の対象（型テンプレを使っても品質バーは免除されない）。本編Cの図解は `diagrams.py`（`diagram_flow_h` 等・全16ビルダー）を pages 経由で使う。実装例は `projects/pages_demo/generate.py`。
