...
# グラフ用トークン（「グラフ集」準拠＝無彩グレー3段＋アクセント1色。
# 配色は The Academy にリブランド：アクセント＝PRIMARY（青）、グレー3段は集の実測値）。
# 設計原則：主役は1系列/1点だけアクセント、残りはグレー。凡例・縦グリッド・枠線は置かない、
# 横罫線は最も薄いグレーのみ。値ラベルは棒端の外に出す（積み上げ時のみ内側白）。
# 無強調色は「純グレー」ではなく Primary Blue と同じ寒色相へ寄せた“青みグレー”に統一。
# 彩度の高い PRIMARY が主役として立ち、残りは青に馴染む中立色として後退する（B>G>R の寒色バイアス）。
CHART_INK    = RGBColor(0x4A, 0x5A, 0x8C)   # 中濃の青（積み上げ先頭・主バー。黒っぽさを避け青寄りに）
CHART_GREY   = RGBColor(0x8C, 0x97, 0xBC)   # 中間の青みグレー
CHART_GREY_L = RGBColor(0xDC, 0xE1, 0xF0)   # 淡い青みグレー（無強調のバー・残りスライス）
CHART_ACCENT = PRIMARY                       # アクセント＝ブランド青（1画面1箇所）
CHART_GRID   = RGBColor(0xED, 0xEF, 0xF8)   # 横罫線（最薄・青み）
CHART_AXIS   = RGBColor(0xC7, 0xCD, 0xE0)   # 軸線（薄い青みグレー）

# 既定パレット（系列／扇の順に適用）。先頭＝アクセント青、以降はグレー濃→淡のランプ。
# 単系列で特定の点だけ強調したいときは各ビルダーの highlight= を使う（系列色より優先）。
CHART_PALETTE = (
    CHART_ACCENT,   # 主役＝ブランド青
    CHART_INK,      # 濃グレー
    CHART_GREY,     # 中グレー
    CHART_GREY_L,   # 淡グレー
    SECONDARY,      # 濃紺（第5系列以降の予備）
    TEXT_MUTED,     # グレー（予備）
)


def _style_plot_area(chart, *, y_grid=True):
    """チャートジャンク除去（集の共通ルール）：縦グリッド無し・枠線無し・軸線最薄・
    目盛なし・横罫線は最薄グレーのみ。value_axis / category_axis の両方を整える。"""
    va, ca = chart.value_axis, chart.category_axis
    # 横罫線（値軸）＝最薄グレー・細線。無しにもできる。
    va.has_major_gridlines = bool(y_grid)
    if y_grid:
        gl = va.major_gridlines.format.line
        gl.color.rgb = CHART_GRID
        gl.width = Pt(0.5)
    ...
    # 軸線：カテゴリ軸は薄い基線、値軸線は消す
    try:
        ca.format.line.color.rgb = CHART_AXIS
    ...
    try:
        ...
        pass
    # プロットエリア／チャート枠の塗り・線を消す（背景を出さない）
    try:
        chart.plots[0].format.fill.background()
    except Exception:
        pass
...
def _reverse_category_axis(chart):
    """カテゴリ軸の並びを反転（横棒で先頭カテゴリを上端に出す＝降順リストの体裁）。"""
    from pptx.oxml.ns import qn
    ...
    # 値軸を上端(x軸)側へ寄せ、ラベルが上下反転しないようにする
    val_ax = chart.value_axis._element
    cross = val_ax.find(qn("c:crosses"))
...
def bar_chart(slide, x, y, w, h, *, categories, series, colors=None,
              ...
              highlight=None, rest_color=CHART_GREY_L, stacked=False,
              reverse_order=False, gap_width=150, overlap=None,
              y_grid=None, number_format=None, font_size=10):
    """棒グラフ（縦／横）を配置する（グラフ集デザイン準拠）。

    デザイン規則（集と一致）：縦グリッド無し・枠線無し・目盛なし、横罫線は最薄グレーのみ。
    単系列は既定でグレー、`highlight` で指定した点だけアクセント青（V1 直近強調・B1 上位強調）。
    凡例は系列数>1 のときだけ下部に。値ラベルは棒端の外（積み上げ時は内側）。

    Args:
        categories: カテゴリ名のリスト（軸ラベル）。
        series: (系列名, 値リスト) のタプルのリスト。単系列＝[("", [..])]。
        colors: 系列ごとの色。省略時は CHART_PALETTE（先頭＝青、以降グレー濃→淡）。
        horizontal: True で横棒、False で縦棒。降順で渡せば B1 降順リストになる。
        highlight: 単系列時に強調する点。int / int のリスト。指定点＝アクセント、
                   残り＝rest_color。系列色より優先（集の「1点だけ強調」）。
        rest_color: highlight 指定時の無強調バー色（既定＝淡グレー BASE-3）。
        stacked: True で積み上げ（S1/S2）。値ラベルは内側中央・白。
        y_grid: 横罫線の有無。None は横棒=False/縦棒=True。
        show_value / show_legend / number_format / font_size: 従来同様。

    Returns:
        Chart オブジェクト。
    """
    if stacked:
        ct = (XL_CHART_TYPE.BAR_STACKED if horizontal
    ...
    plot = chart.plots[0]
    # 棒の太さ・重なり（スタイリッシュに細め＝gap_width大きめ。既定150）
    ...
    hi = {highlight} if isinstance(highlight, int) else (set(highlight) if highlight else set())
    for i, s in enumerate(plot.series):
        base = palette[i % len(palette)]
        s.format.fill.solid()
        s.format.fill.fore_color.rgb = base
        # 単系列＋highlight：点ごとに色を割り当て（指定点＝青／残り＝グレー）
        if single and hi:
            for j, pt in enumerate(s.points):
                pt.format.fill.solid()
                pt.format.fill.fore_color.rgb = CHART_ACCENT if j in hi else rest_color

    _style_axis_fonts(chart, font_size=font_size)
    grid = (not horizontal) if y_grid is None else y_grid
    _style_plot_area(chart, y_grid=grid)
    if reverse_order:
        _reverse_category_axis(chart)
...
def line_chart(slide, x, y, w, h, *, categories, series, colors=None,
               markers=True, marker_size=4, show_value=False, show_legend=None,
               number_format=None, line_width=1.75, font_size=10):
    """折れ線グラフを配置する。
    ...
    # 単系列は集の T1 に合わせ既定＝濃グレー（アクセントは複数線の主役1本に使う）
    palette = colors or ((CHART_INK, CHART_ACCENT, CHART_GREY, CHART_GREY_L)
                         if len(series) == 1 else CHART_PALETTE)
    plot = chart.plots[0]
    for i, s in enumerate(plot.series):
        ...
        if markers:
            s.marker.style = XL_MARKER_STYLE.SQUARE   # 集 T1 に合わせ角マーカー
    ...
    _style_axis_fonts(chart, font_size=font_size)
    _style_plot_area(chart, y_grid=True)
    if show_value:
        plot.has_data_labels = True
...
def _set_doughnut_hole(chart, size):
    """ドーナツの穴サイズ(%)を設定。集の C1(中央数字)=太リング/ C2(ゲージ)=細リングを再現。"""
    from pptx.oxml.ns import qn
    for dPt in chart._chartSpace.iter(qn("c:doughnutChart")):
        hs = dPt.find(qn("c:holeSize"))
        if hs is None:
            hs = dPt.makeelement(qn("c:holeSize"), {})
            dPt.append(hs)
        hs.set("val", str(int(size)))


def pie_chart(slide, x, y, w, h, *, categories, values, colors=None,
              doughnut=True, hole_size=78, highlight=0, show_value=False,
              show_percentage=False, show_legend=False, number_format=None, font_size=10):
    """円／ドーナツグラフを配置する（グラフ集デザイン準拠＝既定ドーナツ）。

    集の円グラフ規則：ドーナツ（中央数字が主役なので中抜き）、扇は先頭＝アクセント青・
    残りグレー、凡例は置かない（ラベルは下・中央数字は別途 textbox で重ねる）。
    2分割ドーナツ（C1）は values=[達成, 残り] を渡し highlight=0 で達成側を青にする。

    Args:
        categories: 扇のラベル。
        values: 値のリスト（categories と同順・同数）。
        colors: 扇ごとの色。省略時は先頭＝青・残り＝グレー濃→淡。
        doughnut: True でドーナツ（既定）、False で円（PIE）。
        hole_size: ドーナツ穴サイズ(%)。既定55（C1中央数字ドーナツ相当）。ゲージ(C2)は76前後の細リング。
        highlight: アクセント青にする扇の index（既定0）。None で色指定に従う。
        show_value / show_percentage: 既定 False（中央数字を別途重ねる想定）。
        show_legend: 既定 False（集は凡例を置かない）。
        number_format / font_size: 従来同様。

    Returns:
        Chart オブジェクト。
    """
    ct = XL_CHART_TYPE.DOUGHNUT if doughnut else XL_CHART_TYPE.PIE
    cd = _build_category_data(categories, [("", values)], number_format)
    chart = _add_chart(slide, ct, x, y, w, h, cd).chart

    # 2分割は集 C1 に合わせ「残り＝淡い青みグレー」。3扇以上は濃→淡ランプ。
    two = len(values) == 2
    ramp = ((CHART_GREY_L,) if two else (CHART_INK, CHART_GREY, CHART_GREY_L, SECONDARY, TEXT_MUTED))
    plot = chart.plots[0]
    for i, point in enumerate(plot.series[0].points):
        ...
        if colors:
            ...
            point.format.fill.fore_color.rgb = CHART_ACCENT
        else:
            # 強調扇を除いた青みグレーランプ（濃→淡）
            gi = i if highlight is None else (i - 1 if i > highlight else i)
            point.format.fill.fore_color.rgb = ramp[gi % len(ramp)]

    if doughnut:
        _set_doughnut_hole(chart, hole_size)
